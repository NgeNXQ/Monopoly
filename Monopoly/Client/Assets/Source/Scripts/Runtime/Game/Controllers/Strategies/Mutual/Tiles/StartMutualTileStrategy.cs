...
{
    internal sealed class StartMutualTileStrategy : ITileStrategy
    {
        public Type TileType { get; } = typeof(StartTile);

        public void Execute(PawnController pawn, Tile tile)
        {
            pawn.TransactTakeBalance(GameManager.Instance.CircleSuperBonus);
            pawn.CompleteTurn();
        }
    }
}
