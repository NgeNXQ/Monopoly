...
namespace Monopoly.Client.Runtime.Game.Controllers.Strategies.Player.Factories
{
    internal sealed class PlayerCardStrategyFactory : ISimpleFactory<ICardStrategy, CardScriptableObject>
    {
        private readonly Dictionary<Type, ICardStrategy> strategies;

        internal PlayerCardStrategyFactory()
        {
            ...
            {
                if (!strategyType.Namespace.Contains("Mutual") && !strategyType.Namespace.Contains("Player"))
                    continue;
